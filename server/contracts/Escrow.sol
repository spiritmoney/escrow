...
contract Escrow {
    enum State { Created, Funded, DeliveryRequested, Delivered, Completed, Refunded, Disputed }
    ...
    event AgreementCreated(uint indexed id, address buyer, address seller, address verifier, uint amount, string description);
    ...
    function newAgreement(address _seller, address _verifier, string memory _description) external payable returns (uint) {
        ...
        emit AgreementCreated(id, msg.sender, _seller, _verifier, msg.value, _description);
        emit Funded(id);
    ...
    function requestDelivery(uint _id) external {
        ...
        require(msg.sender == agreement.seller, "Only seller can request delivery confirmation");
    ...
    function confirmDelivery(uint _id) external {
        Agreement storage agreement = agreements[_id];
        require(msg.sender == agreement.buyer || msg.sender == agreement.verifier, "Only buyer or verifier can confirm delivery");
        require(agreement.state == State.DeliveryRequested, "Delivery not requested yet");
    ...
    function complete(uint _id) external {
        ...
        require(agreement.state == State.Delivered, "Goods/services not delivered");
    ...
    function raiseDispute(uint _id) external {
        Agreement storage agreement = agreements[_id];
        require(msg.sender == agreement.buyer || msg.sender == agreement.seller, "Only buyer or seller can raise dispute");
        require(agreement.state == State.Funded || agreement.state == State.Delivered, "Invalid state for dispute");
        
        agreement.state = State.Disputed;
        emit DisputeRaised(_id);
    }

    function resolveDispute(uint _id, address _winner) external {
        Agreement storage agreement = agreements[_id];
        require(msg.sender == agreement.arbitrator, "Only arbitrator can resolve dispute");
        require(agreement.state == State.Disputed, "No active dispute");
        require(_winner == agreement.buyer || _winner == agreement.seller, "Invalid winner address");
        
        agreement.state = State.Completed;
        payable(_winner).transfer(agreement.amount);
        emit DisputeResolved(_id, _winner);
    }

    function setArbitrator(uint _id, address _arbitrator) external {
        Agreement storage agreement = agreements[_id];
        require(msg.sender == agreement.buyer || msg.sender == agreement.seller, "Only buyer or seller can set arbitrator");
        require(agreement.arbitrator == address(0), "Arbitrator already set");
        
        agreement.arbitrator = _arbitrator;
    }

    function claimTimeout(uint _id) external {
        Agreement storage agreement = agreements[_id];
        ...
        require(block.timestamp >= agreement.createdAt + TIMEOUT, "Timeout not reached");
