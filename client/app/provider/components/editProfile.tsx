...
export function EditVendor({ toggleEdit }: EditProps) {
  ...
  const handleEditToggle = (field: keyof typeof editable) => {
    ...
    setTimeout(() => {
      if (field === "firstName" && firstNameRef.current)
        firstNameRef.current.focus();
      if (field === "lastName" && lastNameRef.current)
        lastNameRef.current.focus();
      if (field === "storeName" && storeNameRef.current)
        storeNameRef.current.focus();
      if (field === "kingschatNumber" && kingschatNumberRef.current)
        kingschatNumberRef.current.focus();
      if (field === "emailAddress" && emailAddressRef.current)
          ...
          <span className="flex space-x-2">
            <input
              ...
              className={`w-full border-2 border-gray-400 rounded-lg p-2 ${
                editable.firstName ? "" : "bg-gray-200"
              }`}
        ...
        <div>
          ...
          <span className="flex space-x-2">
            <input
              ...
              ref={storeNameRef}
              className={`w-full border-2 border-gray-400 rounded-lg p-2 ${
                editable.storeName ? "" : "bg-gray-200"
              }`}
        ...
        <div>
          ...
          <span className="flex space-x-2">
            <input
              ...
              className={`w-full border-2 border-gray-400 rounded-lg p-2 ${
                editable.kingschatNumber ? "" : "bg-gray-200"
              }`}
        ...
        <div>
          ...
          <span className="flex space-x-2">
            <input
              ...
              ref={emailAddressRef}
              className={`w-full border-2 border-gray-400 rounded-lg p-2 ${
                editable.emailAddress ? "" : "bg-gray-200"
              }`}
...
export function EditFreelancer({ toggleEdit }: EditProps) {
  ...
  const decrementRate = () => {
    if (hourlyRate > 1) {
      // Ensure the rate doesn't go below 1
      setHourlyRate((prevRate) => prevRate - 1);
    }
  };
      ...
      <div className="rounded-lg p-7 w-full md:w-[550px] h-full flex flex-col space-y-3">
        ...
        <div>
          <p className="text-[12px] font-semibold">
            What is the title of the service you offer?
          </p>
          <input
            type="text"
            className="w-full border-2 border-gray-400 rounded-lg p-2"
        ...
        <div>
          <p className="text-[12px] font-semibold">
            What is the category of the service you offer?
          </p>
          <select
            name="category"
            className="w-full border-2 border-gray-400 rounded-lg p-2 text-gray-400"
          >
            <option value="">
              <p className="w-80">Category</p>
            </option>
        ...
        <div>
          ...
          <div className="flex items-center justify-between w-full">
            {/* From Year Dropdown */}
            <select
              className="border-2 border-gray-400 w-36 md:w-56 p-3 rounded-lg text-gray-500"
              value={fromYear}
        ...
        <div>
          <div className="w-full">
            ...
            <label
              htmlFor="resume"
              className="w-full border-2 border-dashed border-gray-400 rounded-lg p-2 flex items-center justify-center cursor-pointer"
            >
              <img
